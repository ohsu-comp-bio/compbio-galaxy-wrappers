...
class RecordKeeper:
    ...
    def include_record(self, include):
        ...
        if any(filt in self.rec_filters.filters for filt in include.filters) or \
                ...
                check_info(self.record, include.info):
            return True
        return False

    def keep_record(self, exclude, include, threshold):
        if exclude and not include:
            return not self.exclude_record(exclude, threshold)
        elif include and not exclude:
            return self.include_record(include)
        elif exclude and include:
            if self.include_record(include):
                return True
            else:
                return not self.exclude_record(exclude, threshold)
        else:
            ...
            for i in range(len(args.exclude)):
                ...
                if keep:  # if a record has not been excluded by a preceding exclude argument
                    ...
                    if record.ALT[0].type == 'SNV':
                        keep = recordkeeper.keep_record(exclude, include, float(args.snp_threshold[i]))
                    else:
                        keep = recordkeeper.keep_record(exclude, include, float(args.indel_threshold[i]))
                else:
                    break
