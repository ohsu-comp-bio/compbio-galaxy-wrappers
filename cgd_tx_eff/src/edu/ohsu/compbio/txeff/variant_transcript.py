...
class VariantTranscript(AnnovarVariantFunction):
    ...
    def __str__(self):
        ...
        '''
        return f'[AnnovarVariantFunction: genotype={self.chromosome}-{self.position}-{self.reference}-{self.alt}-transcript={self.cdna_transcript}-{self.protein_transcript}' + ('-splicing' if self.splicing else '') 
    ...
    def get_copy(self):
        ...
        transcript.amino_acid_position = self._noneIfEmpty(self.amino_acid_position)
        transcript.base_position = self._noneIfEmpty(self.base_position)
        transcript.exon = self._noneIfEmpty(self.exon)
        transcript.gene = self._noneIfEmpty(self.gene)
        transcript.sequence_variant = self._noneIfEmpty(self.sequence_variant)
        transcript.c_dot = self._noneIfEmpty(self.c_dot)
        transcript.p_dot1 = self._noneIfEmpty(self.p_dot1)
        transcript.p_dot3 = self._noneIfEmpty(self.p_dot3)
        transcript.splicing = self._noneIfEmpty(self.splicing)
        transcript.cdna_transcript = self._noneIfEmpty(self.cdna_transcript)
    ...
    def __lt__(self, other):
        ...
        self_result = p.search(self.cdna_transcript)
        ...
        other_result = p.search(other.cdna_transcript)
    ...
    def _get_self_score(self):
        ...
        # Protein fields        
        if self.protein_transcript and self.amino_acid_position and self.p_dot1 and self.p_dot3:
            score += 1
