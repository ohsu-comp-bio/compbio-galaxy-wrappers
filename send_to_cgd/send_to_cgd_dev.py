...
def run_cmd(cmd):
    ...
    if stderr:
        ...
        pass
    # Temporary, to deal with RDM samples until we also are able to see the sample id.
    elif json.loads(stdout)['errors'] == [u'Could not find patient to provide previously reported variants']:
        print(json.loads(stdout)['errors'])
        return '[]'
