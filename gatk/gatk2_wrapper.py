...
def __main__():
    ...
    # JHL: Added dir option, to prevent tempfile using HTCondor defined TMPDIR.
    tmp_dir = tempfile.mkdtemp( prefix='tmp-gatk-', dir='' )
...
#        cmd = '%s %s' % ( cmd, ' '.join( map( unhexlify, options.pass_through_options_encoded ) ) )
    ...
    # JHL: Removed cwd option.  Also, why is this using shell True?
    ...
    # JHL: Not necessary, cleanup is handled by Galaxy.
    # cleanup_before_exit( tmp_dir )
